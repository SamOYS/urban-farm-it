...
(function () {
  ...
  theme.CartForm = (function () {
    var selectors = {
      ...
      cartLinkBtn: '.cart-link',
      cartLinkPrice: '.cart-link__price',
      cartLinkCount: '.cart-link__count',
    };
    ...
    function CartForm(form) {
      ...
      this.cartLinkPrice = document.querySelector(selectors.cartLinkPrice);
      this.cartLinkBtn = document.querySelector(selectors.cartLinkBtn);
      this.cartLinkCount = document.querySelector(selectors.cartLinkCount);
    ...
    CartForm.prototype = Object.assign({}, CartForm.prototype, {
      ...
      cartMarkup: function (html) {
        ...
        this.updateCartLinkPrice(subtotal, count);
      ...
      updateCartLinkPrice: function (subtotal, count) {
        if (this.cartLinkPrice) {
          const formattedTotalPrice = theme.Currency.formatMoney(
            subtotal,
            theme.settings.moneyFormat
          );
          this.cartLinkPrice.innerHTML = `${formattedTotalPrice}`;
        }
        if (this.cartLinkCount) {
          this.cartLinkCount.innerHTML = `(${count})`;
        }
      },
      ...
      updateSubtotal: function (subtotal) {
        this.form.querySelector(selectors.subTotal).innerHTML = theme.Currency.formatMoney(
          subtotal,
          theme.settings.moneyFormat
        );
      },
  ...
  theme.collapsibles = (function () {
    ...
    function toggle(evt) {
      ...
      if (isTab) {
        ...
        var newModule;
        document
          .querySelectorAll(selectors.tabs + '[data-id="' + el.dataset.id + '"]')
          .forEach((el) => {
  ...
  class ParallaxImage extends HTMLElement {
    ...
    init() {
      ...
      if (progress > -0.1 && progress < 1.1) {
        ...
        element.style.transform = `translate3d(${position * multipliers[0]}px, ${
          position * multipliers[1]
        }px, 0)`;
      }
  ...
  theme.AjaxProduct = (function () {
    ...
    ProductForm.prototype = Object.assign({}, ProductForm.prototype, {
      addItemFromForm: function (evt) {
        ...
        let bogoSelect = document.getElementById('bogo-product-select');
        if (bogoSelect && bogoSelect.value) {
          items.push({
            id: bogoSelect.value,
            quantity: 1,
          });
        ...
        // Get selected cross sell product ids from the form
        let crossSellInputs = document.querySelectorAll('input[name=cross-sell]:checked');
        for (let input of crossSellInputs) {
          items.push({
            id: input.value,
        ...
        // Custom Product Bundle Products
        const productBundles = Array.from(document.querySelectorAll('product-bundle'));
        const activeProductBundles = productBundles
        ...
        items = [...items, ...activeProductBundles];
  ...
  class ToolTip extends HTMLElement {
    ...
    _open(context, insertedHtml) {
      ...
      document.documentElement.on('click' + '.tooltip-outerclick', (event) => {
        if (this.el.dataset.toolTipOpen === 'true' && !this.inner.contains(event.target))
          this._close();
      });
    ...
    function stickyHeaderHeight() {
      ...
      if (siteHeader.classList.contains('site-header--stuck') && !theme.config.bpSmall) {
        let siteHeaderPadding = parseFloat(
          window.getComputedStyle(siteHeader, null).getPropertyValue('padding-top')
        );
  ...
  theme.buildProductGridItem = function (items, gridWidth, rowOf, imageSize) {
    ...
    items.forEach((product) => {
      ...
      if (theme.settings.predictiveSearchPrice)
        ...
        }${theme.Currency.formatMoney(product.price_min, theme.moneyFormat)}</div>`;
  ...
  class AgeVerificationPopup extends HTMLElement {
    constructor() {
      ...
      if ((this.cookie || this.dataset.enabled === 'false') && this.dataset.testMode === 'false')
        return;
    ...
    StoreAvailability.prototype = Object.assign({}, StoreAvailability.prototype, {
      updateContent: function (variantId) {
        var variantSectionUrl =
          this.baseUrl + '/variants/' + variantId + '/?section_id=store-availability';
  ...
  theme.Collection = (function () {
    ...
    Collection.prototype = Object.assign({}, Collection.prototype, {
      ...
      onFormSubmit: function (evt) {
        ...
        if (el.hasAttribute('filter-search')) {
          return;
        }
      ...
      fetchOpenCollasibleFilters: function () {
        return Array.from(
          document.querySelectorAll(`${selectors.sidebar} ${selectors.trigger}.${classes.isOpen}`)
  ...
  theme.Product = (function () {
    ...
    Product.prototype = Object.assign({}, Product.prototype, {
      ...
      updateCartButton: function (evt) {
        ...
        const productStickyNav = document.getElementById('product-nav');
        const productStickyPrice = productStickyNav
          ? productStickyNav.querySelector('.product__price')
          : null;
        ...
        if (variant) {
          if (variant.available) {
            ...
            var defaultTextSpan = cartBtnText.querySelector('.add-to-cart-text');
            if (defaultTextSpan) {
              defaultTextSpan.textContent = cartBtnText.dataset.defaultText;
            }
            // Update the price part of the button text, if it exists
            if (cartBtnText.childNodes[0].nodeType === Node.TEXT_NODE) {
              cartBtnText.childNodes[0].textContent =
                theme.Currency.formatMoney(variant.price, theme.settings.moneyFormat) + ' | ';
            }
            if (productStickyPrice)
              productStickyPrice.textContent = theme.Currency.formatMoney(
                variant.price,
                theme.settings.moneyFormat
              );
          } else {
      ...
      updateUnitPrice: function (evt) {
        ...
        if (variant && variant.unit_price) {
          this.container.querySelector(this.selectors.unitPrice).innerHTML =
            theme.Currency.formatMoney(variant.unit_price, theme.settings.moneyFormat);
      ...
      initVimeoVideo: function (div) {
        videoObjects[div.id] = new theme.VimeoPlayer(div.id, div.dataset.videoId, {
          videoParent: selectors.videoParent,
          autoplay: false,
          style: div.dataset.videoStyle,
          loop: div.dataset.videoLoop,
        ...
        Object.keys(products).forEach(
          function (key) {
            ...
            product.image_responsive_url =
              theme.recentlyViewed.recent[product.handle].featuredImage;
